...
\begin{itemize}
    ...
    The Customers' creation is split to a different component because it requires custom implementations for emailing and interfacing with UserFactory.
\end{itemize}
...
In the following diagrams, the interactions between components for different functionalities of the system is demonstrated.
UML Sequence Diagrams are used to indicate specific message, data flow and ordering of the specified messages between components.
...
Also, components regarding routing and authorization (unless explicity stated) are omitted for simplication.
...
\end{figure}
\nameref{fig:SDCreateStore} displays the interactions needed to create a new store and its manager by the super user.
...
\subsubsection{SMTP}
The server needs to dispatch emails that doesn't need to be responded to, thus creating mailboxes to collect user emails via protocols such as POP3 or IMAP is not required for the proposed system to function.
